...
func TestPostgresDBRepoAllUsers(t *testing.T) {
	...
	users, err = testRepo.AllUsers()
	if err != nil {
		t.Errorf("all users reports an error on second insert: %s", err)
	}
	if len(users) != 2 {
...
func TestPostgresDBRepoGetUser(t *testing.T) {
	...
	if err == nil {
		t.Errorf("no error reported when getting a user that does not exist ")
	}
